...
3. **Draft Findings**:
   - **Uniqueness**: Ensure every finding is unique. Do not repeat the same issue multiple times (e.g. do not list "Low contrast on CTA" for every single button; group them if needed).
   - **Exhaustive Scope**: Report **EVERY** valid issue you identify. There is NO limit on the number of findings.
   - **Granularity**: Analyze every single element (buttons, text, images, spacing, navigation, forms).
   - For each finding, evaluate:
     - **Section Name**: MANDATORY. You must name the specific UI section (e.g. "Announcement Bar", "Hero", "Footer", "Product Grid"). Do NOT use "General" or "Issue".
     - **Severity**: High (Revenue blocker), Medium (Friction), Low (Polish).
